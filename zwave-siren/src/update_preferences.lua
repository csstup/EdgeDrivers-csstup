...
local function update_preferences(driver, device, args)
  ...
  for id, value in pairs(device.preferences) do
    if not (args and args.old_st_store) or (args.old_st_store.preferences[id] ~= value and preferences and preferences[id]) then
      ...
      elseif preferences[id].type == 'soundvol' then
        -- preference type not currently used, but remains for support in the future
        -- local new_parameter_value = preferencesMap.to_numeric_value(device.preferences[id])
        -- device:send(SoundSwitch:ConfigurationSet({volume = new_parameter_value}))
